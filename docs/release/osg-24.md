...
-   [OpenBao 2.6.2](https://github.com/openbao/openbao/releases/tag/v2.6.2)
    -   Sync up with release in EPEL
        -   Includes a fix for critical severity network-exploitable security vulnerability affecting
            many authentication methods, but does not affect `htvault-config`
...
-   Upcoming
    -   [HTCondor 24.12.23](https://htcondor.readthedocs.io/en/24.x/version-history/feature-versions-24-x.html#version-24-12-23)
        -   Fix bug where FS authentication fails when `TCP_FORWARDING_HOST` is set
        -   Fix bug where `condor_history` might not find a specific cluster or proc
        -   HTCondor tarballs now contain Apptainer 1.5.3 and [Pelican 7.26.0](https://github.com/PelicanPlatform/pelican/releases/tag/v7.26.0)
        -   Removing Pelican server code reduces tarball size by 10 megabytes
...
-   Upcoming:
    -   [HTCondor 24.12.21](https://htcondor.readthedocs.io/en/24.x/version-history/feature-versions-24-x.html#version-24-12-21)
