...
Origin:
  ...
  Exports:
    ...
    - FederationPrefix: "<EXTERNAL OSDF NAMESPACE>"
      StoragePrefix:    "/pelican/<SUBDIRECTORY>"
...
```
where `<SUBDIRECTORY>` will be a mount point for a volume that you want to export.
(If you only have one volume, you may leave `<SUBDIRECTORY>` empty.)
...
| `OUTSIDE_DIRECTORY`   | `/mnt/origin`                    | POSIX origins: the directory outside the container that stores the data to be served  | 
| `INSIDE_DIRECTORY`    | `/pelican`                       | POSIX origins: the path inside the container that files will be served from           |
