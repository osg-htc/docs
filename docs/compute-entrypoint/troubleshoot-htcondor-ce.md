...
- **The job does not match any job routes:**
  ...
  [routes](/compute-entrypoint/job-router-recipes#how-job-routes-are-constructed).
- **The route(s) that the job matches to are full:**
  See [limiting the number of jobs](/compute-entrypoint/job-router-recipes#limiting-the-number-of-jobs).
...
Increase the [debug level](/compute-entrypoint/troubleshoot-htcondor-ce/#htcondor-ce-troubleshooting-items) and consult the
[HTCondor-CE logs](/compute-entrypoint/troubleshoot-htcondor-ce/#htcondor-ce-troubleshooting-data) for errors.
