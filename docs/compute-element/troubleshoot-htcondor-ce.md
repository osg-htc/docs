...
- **The job does not match any job routes:**
  ...
  [routes](job-router-recipes.md#how-job-routes-are-constructed).
- **The route(s) that the job matches to are full:**
  See [limiting the number of jobs](job-router-recipes.md#limiting-the-number-of-jobs).
...
Increase the [debug level](troubleshoot-htcondor-ce.md#htcondor-ce-troubleshooting-items) and consult the
[HTCondor-CE logs](troubleshoot-htcondor-ce.md#htcondor-ce-troubleshooting-data) for errors.
