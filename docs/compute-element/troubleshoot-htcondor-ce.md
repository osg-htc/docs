...
To get around this, you will want to add the attributes to your [job routes](job-router-recipes.md). 
...
1.  **If the MasterLog is filled with `ERROR:SECMAN...TCP connection to collector...failed`:** This is likely due to a misconfiguration for a host with multiple network interfaces. Verify that you have followed the instructions in [this](install-htcondor-ce.md#networking) section of the install guide.
...
The CE will only accept jobs from users that authenticate via [LCMAPS VOMS](../security/lcmaps-voms-authentication.md).
...
- **The job does not match any job routes:**
  ...
  [routes](../compute-element/job-router-recipes.md#how-job-routes-are-constructed).
- **The route(s) that the job matches to are full:**
  See [limiting the number of jobs](../compute-element/job-router-recipes.md#limiting-the-number-of-jobs).
...
[this](install-htcondor-ce.md#limiting-or-disabling-locally-running-jobs-on-the-ce) section of the install documentation.
...
See the [submitting to HTCondor-CE](submit-htcondor-ce.md) document for details.
...
[JOB\_ROUTER\_DEFAULTS](job-router-recipes.md#job_router_defaults) configuration variable):
...
2.  **If your job matches more than one route:** the tool will tell you by showing all matching routes after the job ID:
    ...
    Both routes evaluate to `true` for the job’s ClassAd because it contained `osgTestPBS = true`. Make sure your routes are mutually exclusive, otherwise you may have jobs routed incorrectly! See the [job route configuration page](job-router-recipes.md) for more details.

3.  **If it is unclear why jobs are matching a route:** wrap the route's requirements expression in [debug()](job-router-recipes.md#debugging-routes) and check the [JobRouterLog](#jobrouterlog) for more information.
...
Increase the [debug level](../compute-element/troubleshoot-htcondor-ce.md#htcondor-ce-troubleshooting-items) and consult the
[HTCondor-CE logs](../compute-element/troubleshoot-htcondor-ce.md#htcondor-ce-troubleshooting-data) for errors.
...
-   [Installing HTCondor-CE](install-htcondor-ce.md)
