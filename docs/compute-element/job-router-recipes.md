...
The job router considers jobs in the queue ([condor_ce_q](troubleshoot-htcondor-ce.md#condor_ce_q)) that
...
[route's requirements](job-router-recipes.md#filtering-jobs-based-on).
...
The name of the route will be useful in debugging since it shows up in the output of [condor\_ce\_job\_router\_info](troubleshoot-htcondor-ce.md#condor_ce_job_router_info), the [JobRouterLog](troubleshoot-htcondor-ce.md#jobrouterlog), and in the ClassAd of the routed job, which can be viewed with [condor\_ce\_q](troubleshoot-htcondor-ce.md#condor_ce_q) or [condor\_ce\_history](troubleshoot-htcondor-ce.md#condor_ce_history).
...
To assign jobs to an HTCondor accounting group to manage fair share on your local batch system, we recommend using [UID and ExtAttr tables](install-htcondor-ce.md#htcondor-accounting-groups).
