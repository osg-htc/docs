...
-   Do **not** set the job environment through the JobRouter. Instead, add any changes to the `[Local Settings]` section in `/etc/osg/config.d/40-localsettings.ini` and run osg-configure, as documented [here](../other/configuration-with-osg-configure.md#local-settings).
...
The job router considers jobs in the queue ([condor_ce_q](../compute-element/troubleshoot-htcondor-ce.md#condor_ce_q)) that
...
[route's requirements](../compute-element/job-router-recipes.md#filtering-jobs-based-on).
...
The name of the route will be useful in debugging since it shows up in the output of [condor\_ce\_job\_router\_info](../compute-element/troubleshoot-htcondor-ce.md#condor_ce_job_router_info), the [JobRouterLog](../compute-element/troubleshoot-htcondor-ce.md#jobrouterlog), and in the ClassAd of the routed job, which can be viewed with [condor\_ce\_q](../compute-element/troubleshoot-htcondor-ce.md#condor_ce_q) or [condor\_ce\_history](../compute-element/troubleshoot-htcondor-ce.md#condor_ce_history).
...
This section outlines how to set default job limits, memory, cores, and maximum walltime. For an example on how users can override these defaults, consult [this document](submit-htcondor-ce.md#route-defaults).
...
To assign jobs to an HTCondor accounting group to manage fair share on your local batch system, we recommend using [UID and ExtAttr tables](../compute-element/install-htcondor-ce.md#htcondor-accounting-groups).
